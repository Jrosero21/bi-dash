...
export default function Sidebar() {
  ...
  return (
    <aside
      className="hidden lg:flex w-64 shrink-0 flex-col border-r bg-sidebar supports-[backdrop-filter]:backdrop-blur-xl"
    >
      {/* Brand */}
      ...
      <nav className="flex-1 space-y-1 p-3">
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              <span className={`grid place-items-center rounded-md border p-1.5 ${active ? "border-[hsl(var(--ring)/0.35)]" : ""}`}>
                <Icon className={`h-4 w-4 ${active ? "text-[hsl(var(--chart-1))]" : ""}`} />
              </span>
              <span className="truncate">{label}</span>
              <span className={`ml-auto size-1.5 rounded-full transition-opacity
