...
const revenueSeries = months.map((m, i) => {
  const revenue = 1_800_000 + i * 140_000
...
export const dashboards: Record<string, DashboardConfig> = {
  sample: {
    ...
    description: "Registry-driven dashboard (no page code needed).",
    blocks: [
      {
        kind: "kpi",
        items: [
          { title: "Revenue YTD", value: 12_450_000, format: "currency", delta: 2.8, icon: "dollar" },
          { title: "Active Customers", value: 5_870, delta: 1.5, icon: "users" },
