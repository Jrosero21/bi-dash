...
function formatCurrencyMillions(n: number) {
            ...
            <ScatterChart>
              ...
              <Tooltip
                ...
                formatter={(value: number, name: string) =>
                  name === "spend" ? [formatCurrencyMillions(value), "Annual Spend"] : [`${value}%`, "Performance"]
                }
                labelFormatter={() => `Vendor`}
              />
              <Legend />
          ...
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                ...
                outerRadius={120}
                paddingAngle={5}
                label={({ category, amount }) => `${category}: ${formatCurrencyMillions(amount)}`}
                labelLine={false}
              >
              ...
              <Tooltip
                formatter={(value: number) => [formatCurrencyMillions(value), "Spend"]}
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={contractMetrics}>
              ...
              <YAxis
                yAxisId="right"
                orientation="right"
                tickFormatter={(v) => formatCurrencyMillions(Number(v))}
              />
              <Tooltip
                formatter={(v: number, name: string) =>
                  name === "value" ? [formatCurrency(v), "Contract Value"] : [v, "Contract Count"]
                }
              />
              <Legend />
              <Bar yAxisId="left" dataKey="count" name="Contract Count">
                {contractMetrics.map((_, i) => (
                  <Cell key={`count-${i}`} fill={`hsl(var(--chart-${(i % 4) + 1}))`} />
                ))}
              </Bar>
              <Bar yAxisId="right" dataKey="value" name="Contract Value">
                {contractMetrics.map((_, i) => (
                  <Cell key={`value-${i}`} fill={`hsl(var(--chart-${(i % 4) + 1}))`} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
